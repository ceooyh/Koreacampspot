...
public class SpotController {
	...
	public String findCampSpotView(HttpServletRequest request, HttpServletResponse response) {
		...
		int count = 2563;
		  ...
		  // 페이징 하기 위한 총 개수 구하기 try { StringBuilder urlBuilder = new StringBuilder(
		  "http://api.visitkorea.or.kr/openapi/service/rest/GoCamping/basedList"); URL
		  urlBuilder.append("?" + URLEncoder.encode("ServiceKey","UTF-8") +
		  "=de4n60BId3f9KozHqu47z%2FtxC6YjJEtG0KeMQojtPltNyV702A9d5lltXnQdN7W25Q9R71S0krGaTtdfEIEoQw%3D%3D"
		  ); Service Key urlBuilder.append("&" +
		  URLEncoder.encode("ServiceKey","UTF-8") + "=" + URLEncoder.encode(
		  "de4n60BId3f9KozHqu47z%2FtxC6YjJEtG0KeMQojtPltNyV702A9d5lltXnQdN7W25Q9R71S0krGaTtdfEIEoQw%3D%3D",
		  "UTF-8")); 공공데이터포털에서 받은 인증키 urlBuilder.append("&" +
		  URLEncoder.encode("MobileOS","UTF-8") + "=" + URLEncoder.encode("ETC",
		  "UTF-8")); IOS(아이폰),AND(안드로이드),WIN(윈도우폰),ETC urlBuilder.append("&" +
		  URLEncoder.encode("MobileApp","UTF-8") + "=" + URLEncoder.encode("AppTest",
		  "UTF-8")); 서비스명=어플명 urlBuilder.append("&_type=json"); 서비스명=어플명
		  ...
		  if(conn.getResponseCode() == 200) { JSONArray arr =
		  json.getJSONObject("response").getJSONObject("body").getJSONObject("items").
		  getJSONArray("item"); count = arr.length(); } }catch(
	 ...
	 * E: 페이징 정보
	 * 
	 * try { StringBuilder urlBuilder = new StringBuilder(
	 * "http://api.visitkorea.or.kr/openapi/service/rest/GoCamping/basedList"); /*
	 * URL
	 */
	urlBuilder.append("?"+URLEncoder.encode("ServiceKey","UTF-8")+"=de4n60BId3f9KozHqu47z%2FtxC6YjJEtG0KeMQojtPltNyV702A9d5lltXnQdN7W25Q9R71S0krGaTtdfEIEoQw%3D%3D"); /*
																																										 * Service
																																										 * Key
																																										 */
	urlBuilder.append("&"+URLEncoder.encode("ServiceKey","UTF-8")+"="+URLEncoder.encode("de4n60BId3f9KozHqu47z%2FtxC6YjJEtG0KeMQojtPltNyV702A9d5lltXnQdN7W25Q9R71S0krGaTtdfEIEoQw%3D%3D","UTF-8")); /*
																																																	 * 공공데이터포털에서
																																																	 * 받은
																																																	 * 인증키
																																																	 */
	...
	HttpURLConnection conn = (HttpURLConnection) url
			.openConnection();conn.setRequestMethod("GET");conn.setRequestProperty("Content-type","application/json");System.out.println("Response code: "+conn.getResponseCode());
	BufferedReader rd;if(conn.getResponseCode()>=200&&conn.getResponseCode()<=300)
	{
	...
	{
		JSONArray arr = json.getJSONObject("response").getJSONObject("body").getJSONObject("items")
				.getJSONArray("item");
		...
		for (int i = 0; i < arr.length(); i++) {
			...
			// 리스트에 추가
			list.add(new SpotDTO(contentId, facltNm, lineIntro, intro, bizrno, manageSttus, hvofBgnde, hvofEnddle,
					featureNm, induty, lctCl, doNm, sigunguNm, zipcode, addr1, addr2, tel, homepage, gnrlSiteCo,
					autoSiteCo, glampSiteCo, caravSiteCo, indvdlCaravSiteCo, siteBottomCl1, siteBottomCl2,
					siteBottomCl3, siteBottomCl4, siteBottomCl5, glampInnerFclty, caravInnerFclty, operPdCl,
					trlerAcmpnyAt, caravAcmpnyAt, toiletCo, swrmCo, wtrplCo, brazierCl, sbrsCl, sbrsEtc, posblFcltyCl,
					posblFcltyEtc, exprnProgrm, themaEnvrnCl, eqpmnLendCl, animalCmgCl, tourEraCl, firstImageUrl, star,
					review_count));
		}
	...
	public String spotSearch(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			StringBuilder urlBuilder = new StringBuilder(
					"http://api.visitkorea.or.kr/openapi/service/rest/GoCamping/searchList"); /* URL */
			urlBuilder.append("?" + URLEncoder.encode("ServiceKey", "UTF-8")
					+ "=de4n60BId3f9KozHqu47z%2FtxC6YjJEtG0KeMQojtPltNyV702A9d5lltXnQdN7W25Q9R71S0krGaTtdfEIEoQw%3D%3D"); /*
																															 * Service
																															 * Key
			...
			urlBuilder.append("&" + URLEncoder.encode("pageNo", "UTF-8") + "="
					+ URLEncoder.encode(pageNo, "UTF-8")); /* 현재 페이지번호 */
			...
			urlBuilder.append("&" + URLEncoder.encode("keyword", "UTF-8") + "="
					+ URLEncoder.encode(search, "UTF-8")); /* 검색 요청할 키워드(인코딩 필요) */
			urlBuilder.append("&_type=json"); /* 서비스명=어플명 */
			...
			if (conn.getResponseCode() == 200) {
				...
				for (int i = 0; i < arr.length(); i++) {
					...
					// 리스트에 추가
					list.add(new SpotDTO(contentId, facltNm, lineIntro, intro, bizrno, manageSttus, hvofBgnde,
							hvofEnddle, featureNm, induty, lctCl, doNm, sigunguNm, zipcode, addr1, addr2, tel, homepage,
							gnrlSiteCo, autoSiteCo, glampSiteCo, caravSiteCo, indvdlCaravSiteCo, siteBottomCl1,
							siteBottomCl2, siteBottomCl3, siteBottomCl4, siteBottomCl5, glampInnerFclty,
							caravInnerFclty, operPdCl, trlerAcmpnyAt, caravAcmpnyAt, toiletCo, swrmCo, wtrplCo,
							brazierCl, sbrsCl, sbrsEtc, posblFcltyCl, posblFcltyEtc, exprnProgrm, themaEnvrnCl,
							eqpmnLendCl, animalCmgCl, tourEraCl, firstImageUrl, star, review_count));
				}
